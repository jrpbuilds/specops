...
- [x] 11.5 Run real smoke workflows with at least two materially different model families; record prompt-following failures, tool-call failures, invalid artifact rates, correction success, review quality, repair success, and user friction; fix prompts before adding orchestration complexity — completed in Phase 11: interactive two-model-family smoke (DeepSeek-V4-Flash deepseek family + GLM-5.2 glm family) against the configured Openference provider; both families produced valid planning artifacts and bound the dual review identity; both returned a legitimate `changes-required` verdict the deterministic gate correctly rejects; evidence recorded in `docs/internal/specops-1.0-model-smoke.md`, harness in `scripts/model-smoke.mjs`; automatic-correction, repair, and archive transitions marked not-exercised (covered by the deterministic V1 workflow suites)
...
- [x] 11.8 Verify Phase 10: every required end-to-end scenario has evidence; two model families complete representative workflows; documentation matches runtime; packed installation succeeds; no generated or bundled custom schema remains; the version is ready to become `1.0.0 — unblocked by the completed task 11.5 (real two-model-family interactive smoke)
...
- [x] 12.2.1 Reproduce B-02: add a regression test proving a review's reviewed identity excludes reviewed OpenSpec planning artifacts, so changing a reviewed planning artifact after review does not invalidate the review - amended: the architectural amendment (design.md §B-02) introduces a separate planning-artifact identity; the implementation identity intentionally excludes planning artifacts; the B-02 reproduction tests the new dual-identity contract
- [x] 12.2.2 Repair B-02: introduce a separate deterministic planning-artifact identity covering proposal/specs/design/tasks; persist it alongside the implementation identity in run state and review artifact; review freshness, verified-state freshness, and completion preflight require both identities to match; mutation to reviewed planning artifacts invalidates review freshness without affecting implementation validation evidence
